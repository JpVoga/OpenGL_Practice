...
namespace Math {
    ...
    struct Vector: public VectorBase<T, count> {
        ...
        static constexpr void buildFromData(Vector<T, count> &v, const auto &data, size_t dataCount) {
            const size_t minCount = std::min(count, dataCount);
            size_t i = 0;
        ...
        constexpr Vector(std::initializer_list<T> values) {
            ...
            size_t i = 0;
            for (const auto &v: values) (*this)[i++] = v;
            for (; i < count; ++i) (*this)[i] = static_cast<T>(0);
        }

        template<numericType ValueT, size_t valuesCount>
        explicit (valuesCount > count)
        constexpr Vector(const std::array<ValueT, valuesCount> &values) {
            buildFromData((*this), values, values.size());
        }

        template<numericType ValueT>
        explicit constexpr Vector(const std::vector<ValueT> &values) {
            buildFromData((*this), values, values.size());
        }
        ...
        constexpr Vector(const Vector<OtherT, otherCount> &other) {
            buildFromData((*this), other, other.getCount());
        }
    ...
    using Vector1 = Vector<float32_t, 1>;
    using Vector2 = Vector<float32_t, 2>;
    using Vector3 = Vector<float32_t, 3>;
    using Vector4 = Vector<float32_t, 4>;
    using Vector5 = Vector<float32_t, 5>;

    using Vector1Flt64 = Vector<float64_t, 1>;
    using Vector2Flt64 = Vector<float64_t, 2>;
    using Vector3Flt64 = Vector<float64_t, 3>;
    using Vector4Flt64 = Vector<float64_t, 4>;
    using Vector5Flt64 = Vector<float64_t, 5>;
