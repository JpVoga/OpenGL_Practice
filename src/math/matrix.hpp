...
namespace Math {
    ...
    struct Matrix {
        ...
        public:
            ...
            static constexpr void buildFromScalarData(Matrix<T, rows, cols> &m, const auto data, size_t dataCount) {
                size_t i = 0;
                for (size_t x = 0; x < rows; ++x) {
                    for (size_t y = 0; y < cols; ++y) {
                        if (i < dataCount) m.at(x, y) = static_cast<T>(data[i++]);
                        else m.at(x, y) = static_cast<T>(0);
                    }
                }
            }
            ...
            constexpr Matrix(std::initializer_list<T> values) {
                ASSERT(values.size() <= getTotalCount());
            ...
            constexpr Matrix(std::initializer_list<RowType> values) {
                ASSERT(values.size() <= rows);

                size_t i = 0;
                for (const auto &value: values) this->setRow(i++, value);
                for (; i < rows; ++i) this->setRow(i, RowType::zero);
            }

            template<numericType ValueT, size_t valueCount>
            explicit ((valueCount > getTotalCount()) || !(std::is_convertible_v<ValueT, T>))
            constexpr Matrix(const std::array<ValueT, valueCount> &values) {
                buildFromScalarData((*this), values, values.size());
            }

            template<numericType ValueT>
            explicit constexpr Matrix(const std::vector<ValueT> &values) {
                buildFromScalarData((*this), values, values.size());
            }

            template<numericType ValueT>
            explicit (!std::is_convertible_v<ValueT, T>)
            constexpr Matrix(const std::array<Math::Vector<ValueT, cols>, rows> &values) {
                for (size_t i = 0; i < rows; ++i) this->setRow(i, RowType(values[i]));
            }

            template<numericType ValueT>
            explicit constexpr Matrix(const std::vector<Math::Vector<ValueT, cols>> &values) {
                const size_t minCount = std::min(values.size(), rows);
                for (size_t i = 0; i < minCount; ++i) this->setRow(i, RowType(values[i]));
                for (size_t i = minCount; i < rows; ++i) this->setRow(i, RowType::zero);
            }
            ...
            [[nodiscard]] constexpr const T &at(size_t row, size_t col) const {
    ...
    using Matrix1   = Matrix<float32_t, 1, 1>;
    using Matrix1x2 = Matrix<float32_t, 1, 2>;
    using Matrix1x3 = Matrix<float32_t, 1, 3>;
    using Matrix1x4 = Matrix<float32_t, 1, 4>;
    using Matrix1x5 = Matrix<float32_t, 1, 5>;
    using Matrix2x1 = Matrix<float32_t, 2, 1>;
    using Matrix2   = Matrix<float32_t, 2, 2>;
    using Matrix2x3 = Matrix<float32_t, 2, 3>;
    using Matrix2x4 = Matrix<float32_t, 2, 4>;
    using Matrix2x5 = Matrix<float32_t, 2, 5>;
    using Matrix3x1 = Matrix<float32_t, 3, 1>;
    using Matrix3x2 = Matrix<float32_t, 3, 2>;
    using Matrix3   = Matrix<float32_t, 3, 3>;
    using Matrix3x4 = Matrix<float32_t, 3, 4>;
    using Matrix3x5 = Matrix<float32_t, 3, 5>;
    using Matrix4x1 = Matrix<float32_t, 4, 1>;
    using Matrix4x2 = Matrix<float32_t, 4, 2>;
    using Matrix4x3 = Matrix<float32_t, 4, 3>;
    using Matrix4   = Matrix<float32_t, 4, 4>;
    using Matrix4x5 = Matrix<float32_t, 4, 5>;
    using Matrix5x1 = Matrix<float32_t, 5, 1>;
    using Matrix5x2 = Matrix<float32_t, 5, 2>;
    using Matrix5x3 = Matrix<float32_t, 5, 3>;
    using Matrix5x4 = Matrix<float32_t, 5, 4>;
    using Matrix5   = Matrix<float32_t, 5, 5>;

    using Matrix1Flt64   = Matrix<float64_t, 1, 1>;
    using Matrix1x2Flt64 = Matrix<float64_t, 1, 2>;
    using Matrix1x3Flt64 = Matrix<float64_t, 1, 3>;
    using Matrix1x4Flt64 = Matrix<float64_t, 1, 4>;
    using Matrix2x1Flt64 = Matrix<float64_t, 2, 1>;
    using Matrix2Flt64   = Matrix<float64_t, 2, 2>;
    using Matrix2x3Flt64 = Matrix<float64_t, 2, 3>;
    using Matrix2x4Flt64 = Matrix<float64_t, 2, 4>;
    using Matrix3x1Flt64 = Matrix<float64_t, 3, 1>;
    using Matrix3x2Flt64 = Matrix<float64_t, 3, 2>;
    using Matrix3Flt64   = Matrix<float64_t, 3, 3>;
    using Matrix3x4Flt64 = Matrix<float64_t, 3, 4>;
    using Matrix4x1Flt64 = Matrix<float64_t, 4, 1>;
    using Matrix4x2Flt64 = Matrix<float64_t, 4, 2>;
    using Matrix4x3Flt64 = Matrix<float64_t, 4, 3>;
    using Matrix4Flt64   = Matrix<float64_t, 4, 4>;
    using Matrix5x1Flt64 = Matrix<float64_t, 5, 1>;
    using Matrix5x2Flt64 = Matrix<float64_t, 5, 2>;
    using Matrix5x3Flt64 = Matrix<float64_t, 5, 3>;
    using Matrix5x4Flt64 = Matrix<float64_t, 5, 4>;
    using Matrix5Flt64   = Matrix<float64_t, 5, 5>;
